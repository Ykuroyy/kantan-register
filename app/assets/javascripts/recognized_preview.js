...
function displayRecognizedPreview() {
  ...
  if (imgData && imgEl) {
    ...
    // sessionStorage.removeItem("capturedImage"); // 必要に応じて表示後に削除。問題が解決したらコメントを外すことを検討
    console.log("[recognized_preview.js] Preview image set and displayed."); // 成功ログはコンソールに残す
  } else if (imgEl) { // imgElは存在するが、imgDataがない場合
    // alert("[recognized_preview.js] 警告: sessionStorageに画像データがありませんでした。img要素は存在します。");
    console.warn("[recognized_preview.js] 警告: sessionStorageに画像データがありませんでした。img要素は存在します。");
  } else {
    // alert("[recognized_preview.js] 警告: id='recognized-preview-image' のimg要素が見つかりませんでした。");
    console.warn("[recognized_preview.js] 警告: id='recognized-preview-image' のimg要素が見つかりませんでした。");
  }
}
function setupPreviewDisplayListeners() {
  ...
  window.addEventListener("pageshow", function(event) {
    ...
    if (event.persisted) {
      console.log("[recognized_preview.js] Page restored from bfcache, attempting to display preview.");
